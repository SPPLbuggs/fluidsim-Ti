    ...
    real(8), allocatable :: ki(:,:,:), ni(:,:,:), Ti(:,:), mui(:,:), &
                            ke(:,:,:), ne(:,:,:), &
                            km(:,:,:), nm(:,:,:), &
                            kt(:,:,:), nt(:,:,:)
    ...
    ! variables
    public  :: ni, Ti, mui, ne, nm, nt, tau_m
...
! *** Particle Initialization ***
    ...
    allocate( ki(g%bx+2, g%by+2, 5), ni(g%bx+2, g%by+2, 3), &
              Ti(g%bx+2, g%by+2), mui(g%bx+2, g%by+2), &
...
! *** Particle Continuity ***
    ...
    if (stage == 5) then
        tau_m = min(tau_m, 1.0 / (get_mue(Te) * ne(i,j,2) + mui(i,j) * ni(i,j,2)))
    end if
...
! *** Calculate Ion Density Flux ***
    ...
    real(8) :: a, Ex(2), Ey(2), mui_pl, mui_mi, vi
    ...
    if (g%type_x(i-1,j-1) == 0) then
        ! rates and coeffs
        mui_mi = 0.5 * (mui(i,j) + mui(i-1,j))
        mui_pl = 0.5 * (mui(i+1,j) + mui(i,j))
        
        ! Flux at i - 1/2
        call get_flux(fluxi_x(1), 1, Ex(1), mui_mi, g%dx(i-1),&
                      ni(i-1:i,j,2), Ti(i-1:i,j))

        ! Flux at i + 1/2
        call get_flux(fluxi_x(2), 1, Ex(2), mui_pl, g%dx(i-1),&
                      ni(i:i+1,j,2), Ti(i:i+1,j))
    ...
    ! - left -
        ...
        if (g%type_x(i-1,j-1) == -2) then
            ...
            vi = sqrt((16.0 * e * phi0 * Ti(i,j)) / (3.0 * pi * me)) * t0 / x0
            
            ! Flux at i - 1/2
            fluxi_x(1) = (2 * a - 1) * mui(i,j) * Ex(1) * ni(i,j,2) &
                         - 0.5 * vi * ni(i,j,2)
    ...
    ! - right -
        ...
        if (g%type_x(i-1,j-1) == 2) then
            ...
            ! Flux at i + 1/2
            fluxi_x(2) = (2 * a - 1) * mui(i,j) * Ex(2) * ni(i,j,2) &
                         + 0.5 * vi * ni(i,j,2)
    ...
    if (g%ny > 1) then
        ...
        if (g%type_y(i-1,j-1) == 0) then
            ! rates and coeffs
            mui_mi = 0.5 * (mui(i,j) + mui(i,j-1))
            mui_pl = 0.5 * (mui(i,j+1) + mui(i,j))
            
            ! Flux at j - 1/2
            call get_flux(fluxi_y(1), 1, Ey(1), mui_mi, g%dy(j-1),&
                          ni(i,j-1:j,2), Ti(i,j-1:j))

            ! Flux at j + 1/2
            call get_flux(fluxi_y(2), 1, Ey(2), mui_pl, g%dy(j-1),&
                          ni(i,j:j+1,2), Ti(i,j:j+1))
        ...
        ! - right -
            ...
            if (right_wall) then
                ...
                fluxi_y(2) = (2 * a - 1) * mui(i,j) * Ey(2) * ni(i,j,2) &
                             + 0.5 * vi * ni(i,j,2)
            else
                fluxi_y(2) = 0
            end if
...
! *** Update Electron Flux ***
    ...
    real(8) :: a, Te(3), mue(2), mut(2), ve, Ex(2), Ey(2), fluxi, vi, ng
    ...
    if (g%type_x(i-1,j-1) == 0) then
        ...
        ! Flux at i - 1/2
        call get_flux(fluxe_x(1), -1, Ex(1), mue(1), g%dx(i-1), &
                       ne(i-1:i,j,2), Te(1:2))
        call get_fluxt(fluxt_x(1), fluxe_x(1), mut(1), g%dx(i-1), &
                       nt(i-1:i,j,2), Te(1:2))

        ! Flux at i + 1/2
        call get_flux(fluxe_x(2), -1, Ex(2), mue(2), g%dx(i), &
                       ne(i:i+1,j,2), Te(2:3))
    ...
    ! - left -
        ...
        if (g%type_x(i-1,j-1) == -2) then
            ...
            ! Flux at i - 1/2
            fluxi = (1 - 2 * a) * mui(i,j) * Ex(1) * ni(i,j,2) &
                    - 0.5 * vi * ni(i,j,2)
            
            ng = (1 - a) / mue(1) * (gam * (1 - 2 * a) &
                 + 0.5 * sqrt(2.0 / (3.0 * pi))) * mui(i,j) * ni(i,j,2)
            fluxe_x(1) = - (2 * a - 1) * mue(1) * Ex(1) * ne(i,j,2) &
                         - 0.5 * ve * (ne(i,j,2) - ng) &
                         - 2 * (1 - a) * gam * fluxi
            
            ng = ng * Ti(i,j)
            fluxt_x(1) = - (2 * a - 1) * mut(1) * Ex(1) * nt(i,j,2) &
                         - 2.0/3.0 * ve * (nt(i,j,2) - ng) &
                         - 2 * (1 - a) * gam * Ti(i,j) * fluxi
    ...
    ! - right -
        ...
        if (g%type_x(i-1,j-1) == 2) then
            ...
            ! Flux at i + 1/2
            fluxi = (1 - 2 * a) * mui(i,j) * Ex(2) * ni(i,j,2) &
                    + 0.5 * vi * ni(i,j,2)
            
            ng = (1 - a) / mue(2) * (gam * (1 - 2 * a) &
                 + 0.5 * sqrt(2.0 / (3.0 * pi))) * mui(i,j) * ni(i,j,2)
            fluxe_x(2) = - (2 * a - 1) * mue(2) * Ex(2) * ne(i,j,2) &
                         + 0.5 * ve * (ne(i,j,2) - ng) &
                         - 2 * (1 - a) * gam * fluxi
            
            ng = ng * Ti(i,j)
            fluxt_x(2) = - (2 * a - 1) * mut(2) * Ex(2) * nt(i,j,2) &
                         + 1.0/3.0 * ve * (nt(i,j,2) - ng) &
                         - 2 * (1 - a) * gam * Ti(i,j) * fluxi
    ...
    if (g%ny > 1) then
        ...
        if (g%type_y(i-1,j-1) == 0) then
            ...
            ! Flux at j - 1/2
            call get_flux(fluxe_y(1), -1, Ey(1), mue(1), g%dy(j-1), &
                           ne(i,j-1:j,2), Te(1:2))
            call get_fluxt(fluxt_y(1), fluxe_y(1), mut(1), g%dy(j-1), &
                           nt(i,j-1:j,2), Te(1:2))

            ! Flux at j + 1/2
            call get_flux(fluxe_y(2), -1, Ey(2), mue(2), g%dy(j), &
                           ne(i,j:j+1,2), Te(2:3))
        ...
        ! - right -
            ...
            if (right_wall) then
                ...
                fluxe_y(2) = - (2 * a - 1) * mue(2) * Ey(2) * ne(i,j,2) &
                             + 0.5 * ve * ne(i,j,2)
            
                fluxt_y(2) = - (2 * a - 1) * mut(2) * Ey(2) * nt(i,j,2) &
                             + 1.0/3.0 * ve * nt(i,j,2)
            else
                fluxe_y(2) = 0
                fluxt_y(2) = 0
...
! *** Update Ion Temperature and Mobility ***
    subroutine upd_Timui(g, phi)
    type(grid), intent(in) :: g
    real(8), intent(in) :: phi(:,:)
    integer :: i,j
    real(8) :: Ex = 0, Ey = 0, E_m
    
    do j = 2, g%by+1
        do i = 2, g%bx+1
            Ex = -0.5 * ((phi(i,j) - phi(i-1,j)) / g%dx(i-1) &
                        +(phi(i+1,j) - phi(i,j)) / g%dx(i))
            
            if (g%ny > 1) then
                Ey = -0.5 * ((phi(i,j) - phi(i,j-1)) / g%dy(j-1) &
                            +(phi(i,j+1) - phi(i,j)) / g%dy(j))
            end if
            
            E_m = abs(Ex) + abs(Ey)
        
            mui(i,j) = get_mui(E_m)
            Ti(i,j)  = get_Ti(mui(i,j), E_m)
        end do
    end do
    
    call comm_real(g%bx, g%by, Ti)
    call comm_real(g%bx, g%by, mui)
    end subroutine
    end module
