...
#include <petsc/finclude/petscksp.h>
    ...
    ! case properties
    real(8), parameter:: Tg0    = 350, & ! kelvin
                         p      = 3,   & ! torr
    ...
    call MPI_File_Open(comm, path//'ti.dat', amode,  info, fh, ierr)
    if (ierr .ne. MPI_SUCCESS) then
        if (my_id == 0) call MPI_File_Delete(path//'ti.dat', info, ierr);
        call MPI_File_Open(comm, path//'ti.dat', amode,  info, fh, ierr)
    end if
    call MPI_File_Close(fh, ierr)
    
    call MPI_File_Open(comm, path//'mui.dat', amode,  info, fh, ierr)
    if (ierr .ne. MPI_SUCCESS) then
        if (my_id == 0) call MPI_File_Delete(path//'mui.dat', info, ierr);
        call MPI_File_Open(comm, path//'mui.dat', amode,  info, fh, ierr)
    end if
    call MPI_File_Close(fh, ierr)
