...
temp = np.fromfile(path + '/ti.dat',dtype=float)
ti = temp.reshape([ts,ny,nx])
ti = ti / 1.5

temp = np.fromfile(path + '/mui.dat',dtype=float)
mui = temp.reshape([ts,ny,nx])
...
if ti.max() > 0.01:
    fig,axes = plt.subplots(1,2,sharey=True,figsize=[8,3])
    (ax1,ax2) = axes
    ax1.plot(ti[tloc[0],yloc,:],x,label='{:.1f}us'.format(t[tloc[0]]), color = colors[0])
    ax1.plot(ti[tloc[1],yloc,:],x,label='{:.1f}us'.format(t[tloc[1]]), color = colors[1])
    ax1.plot(ti[tloc[2],yloc,:],x,label='{:.1f}us'.format(t[tloc[2]]), color = colors[2])
    ax1.set_xlabel('Temperature (eV)')
    ax1.set_xscale('log')
    #ax1.set_xlim([0.2, 150])
    ax1.set_ylim(ylim)
    ...
    v = np.linspace(-3,2.5,30)
    im = ax2.contourf(tt,xx,np.maximum(np.log10(ti[:,yloc,:]),v[0]),v,
                      cmap='plasma', zorder=-20)
    ax2.set_xlabel('Time ($\mu$s)')
    plt.suptitle('(f) Ion Temperature (T$_i$)')
    v = np.arange(-3,3,1)
    clb = fig.colorbar(im, ticks=v)
    clb.ax.set_title('(log$_{10}$eV)')
    ax2.set_xscale('log')
    ...
    if (sv):
        plt.savefig('figures/ti_streak.eps', dpi=300, frameon=None)

# *** Mui Plot ***
if mui.max() > 0.01:
    fig,axes = plt.subplots(1,2,sharey=True,figsize=[8,3])
    (ax1,ax2) = axes
    ax1.plot(mui[tloc[0],yloc,:],x,label='{:.1f}us'.format(t[tloc[0]]), color = colors[0])
    ax1.plot(mui[tloc[1],yloc,:],x,label='{:.1f}us'.format(t[tloc[1]]), color = colors[1])
    ax1.plot(mui[tloc[2],yloc,:],x,label='{:.1f}us'.format(t[tloc[2]]), color = colors[2])
    ax1.set_xlabel(r'Mobility ($m^2/Vs$)')
    #ax1.set_xscale('log')
    #ax1.set_xlim([0.2, 150])
    ax1.set_ylim(ylim)
    ax1.set_yticks(yticks)
    ...
    v = np.linspace(0.04,0.13,30)
    im = ax2.contourf(tt,xx,np.maximum(mui[:,yloc,:],v[0]),v,
                      cmap='plasma', zorder=-20)
    ax2.set_xlabel('Time ($\mu$s)')
    plt.suptitle(r'(g) Ion Mobility ($\mu_i$)')
    v = np.arange(0.04, 0.131, 0.015)
    clb = fig.colorbar(im, ticks=v)
    clb.ax.set_title(r'($m^2/V s$)')
    ax2.set_xscale('log')
    #ax2.set_rasterization_zorder(-10)
    
    if (sv):
        plt.savefig('figures/mui_streak.eps', dpi=300, frameon=None)
